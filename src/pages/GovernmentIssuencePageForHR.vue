<template>
    ...
    >
      ...
      <q-card style="max-width: 500px;">
        <q-card-section>
          ...
          <div class="text-subtitle">Get your Identification credential! Normally this would be a government's page, with a login using the user's
             social security number and password. But this is not an official government backed project, so Alice will get their credential purely by entering this page.
          </div>
      ...
      <q-input class="q-ma-sm" rounded outlined  label="Date of birth" readonly v-model="bob.birthday"/>
        ...
        <q-card-actions>
          ...
          <q-space></q-space>
          <q-btn color="primary" @click="handleCredentialIssuence" :disable="!connected" v-if="!issued">Get Alice's ID<q-tooltip v-if="!connected">You need to connect your wallet first.</q-tooltip></q-btn>
      ...
      <div class="text-body2">
        You have successfully received the credential as Alice. You can check your wallet to see the issued credential, then go to CodeNimbus to receive your Hiring Manager credential.
      </div>
  ...
  const bob = ref({name: "Alice", surname: 'Alicesson', ssn: '121212-1212', birthday: '01.01.2000'});
