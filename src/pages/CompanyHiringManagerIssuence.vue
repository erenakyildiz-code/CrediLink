    ...
    <q-page class="flex flex-center">
      ...
      <div class="row justify-between">
        <div class="column">
          ...
          <div class="text-h5">Welcome to CodeNimbus</div>
        <div class="text-body2">
            You will be receiving you HR credential here, this is for credilink to understand that you are from CodeNimbus.
        </div>
        </div>
        <img src="codeNimbus.webp" style="width: 200px; border-radius: 10px;">
      </div>
    </q-step>
      <q-step
        :name="2"
      ...
      <div class="text-h5" >Prove that you are Alice</div>
      <div class="text-body2">CodeNimbus has Alice's social security number in their database, so they will want you to prove that your ID credential has the same SSN. </div>
      ...
      <q-btn v-if="!proven" label="Proof request" color="primary" @click="startProofFlow" ></q-btn><q-chip v-else label="User proved they have SSN same as Alice!"></q-chip>
    </q-step>
      <q-step
        :name="4"
        title="Receive HRCredential credential"
        icon="download"
        :done="step > 4"
      >
      <div class="text-h5" >Generate and receive your diploma</div>
      <div class="text-body2">Code nimbus will now create the credential</div>
      <div class="text-body2">The schema and the credential definition will also be created here, as before this would have been done by the admins of Code Nimbus. (or the government would create the schema and CodeNimbus would create the cred_def)</div>
      <br>
      <q-btn-group class="q-mt-lg" v-if="diplomaCredFlow != 5"> 
        <q-btn label="Generate HRCredential schema" color="primary" :disable="diplomaCredFlow != 1" @click="generateDiplomaSchema"></q-btn>
        <q-btn label="Generate/GET HRCredential credential definition" color="primary" :disable="diplomaCredFlow != 2" @click="generateCredDef"></q-btn>
        <q-btn label="Connect to wallet" color="primary" :disable="diplomaCredFlow != 3" @click="connect"></q-btn>
        <q-btn label="Receive HRCredential" color="primary" :disable="diplomaCredFlow != 4" @click="giveDiplomaCredential"></q-btn>
      </q-btn-group>
      <q-chip v-else label="HRCredential credential received!"></q-chip>
    </q-step>
      <q-step
        :name="5"
        title="Done"
        ...
        :done="step >5"
      >
      <div class="text-h5" >You have received your HR credential</div>

      <div class="text-body2">You can now create job listings in crediLink using this credential to prove you are from codeNimbus.</div>
    </q-step>
